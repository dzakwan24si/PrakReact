...
export default function Header() {
  ...
  // Ambil inisial dari nama untuk avatar default
  const getInitials = () => {
    ...
    <div className="flex justify-between items-center p-4">
      ...
      <div className="flex items-center space-x-4">
        <div className="relative p-3 bg-blue-100 rounded-2xl text-biru cursor-pointer hover:bg-blue-200 transition">
          <FaBell />
          <span className="absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2 bg-biru text-white rounded-full px-2 py-0.5 text-[10px] font-bold shadow-sm">
            50
          </span>
        </div>
        ...
        <div className="flex items-center space-x-4 border-l pl-4 border-garis">
          <span className="text-teks-samping text-sm">
            Hello, <b className="text-teks">{displayName}</b>
            {profile && (
              <span className="ml-2 text-[10px] bg-hijau/10 text-hijau px-2 py-0.5 rounded-full font-bold uppercase">
                {profile.role}
              </span>
            )}
          </span>
          {/* Avatar dengan inisial jika tidak ada foto */}
          <div className="w-10 h-10 rounded-full bg-hijau text-white flex items-center justify-center font-bold text-sm border border-garis shadow-sm">
            {getInitials()}
          </div>
        </div>
      </div>
